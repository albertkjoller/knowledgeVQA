...
class Qlarifais(BaseModel):
    ...
    def build(self):
        ...
        if self.config.attention.use:
            ...
            self.num_features = self.config.attention.params.num_features
            # defining model
    ...
    def forward(self, sample_list):
        ...
        if self.config.attention.use:
            ...
            if self.config.attention.type == 'question_guided':
                ...
                image_features, attention = self.attention_model(image_features, question_features, self.num_features)
                # image_features have attention multiplied
                print('image_features', image_features.shape)
                print('image_features', attention.shape)
                # TODO: sum features??
