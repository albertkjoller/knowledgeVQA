...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 27 21:12:09 2022
...
def multimodal_saliency(model,
    ...
    try:
        model.image_tensor.grad.data.zero()
    ...
    with context_builder():
        output = model.classify(image_path, input_question, explain=True)
        backward_gradient = get_backward_gradient(output, target)
        output.backward(backward_gradient)

    # TODO: also check this out
    # Get saliency map from gradient.
    saliency_map = gradient_to_saliency(model.image_tensor) 


    # Resize saliency map.
    saliency_map = resize_saliency(model.image_tensor,
                                   saliency_map,
                                   resize,
                                   mode=resize_mode)
