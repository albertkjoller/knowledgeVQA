...
if __name__ == "__main__":
    # getting configs
    config_path = 'mmexp/methods/bounding_boxes/up-down_not_norm.yaml'
    config = OmegaConf.load(os.path.dirname(os.getcwd()) + "/" + config_path)

    # loading images
    general_path = "/imgs/protocol/"
    image_file_names = ["sheep.jpg"]  # , "sheep.jpg"]
    ...
    for file_name in image_file_names:
        # path to image
        image_path = os.path.dirname(os.getcwd()) + general_path + file_name
    ...
    data_loader = build_detection_test_loader_for_images(vision_module.cfg, image_path)
    ...
    for idx, inputs in enumerate(tqdm.tqdm(data_loader)):
        ...
        # use the full model
        outputs = vision_module.grid_feats_vqa(processed_images)
        print('from outputs:', outputs[0]["instances"])


    for idx, img in enumerate(processed_images):
        img = processed_images[idx]['image'].permute(1, 2, 0) # correct shape
        ...
        plt.tight_layout()
        plt.axis('off')
        plt.savefig(f'{os.path.dirname(os.getcwd())}/mmexp/methods/bounding_boxes/imgs/{exp_name}_{image_file_names[idx]}', bbox_inches='tight')
